...
def generate_cad_with_cargo(algo_data: dict, output_filename: str = "loading_result.dxf"):
    """
    根据算法输出的坐标字典，生成 CAD 图纸
    algo_data: 包含 cargoPosition 和 bypassBoardPosition
    output_filename: 输出的 dxf
    """
    # 初始化 CAD 画布
    doc = ezdxf.new('R2010')
    msp = doc.modelspace()
    ...
    # 画旁通板
    # 获取算法生成的 bypassBoardPosition 列表
    ...
    for item in cargo_items:
        ...
        for i in range(1, 5):
            pt = coords_dict.get(str(i), {"x": 0, "y": 0})
            pts.append((pt["x"] + offset_x, pt["y"] + offset_y))
        pts.append(pts[0])

        # 根据货物类型分配颜色
        c_label = item.get("cargoLabel", "Unknown").lower()
        if "tower" in c_label:
            c_color = 3  # 绿色
        elif "nacelle" in c_label:
            c_color = 1  # 红色
        elif "blade" in c_label:
            c_color = 4  # 青色
        elif "hub" in c_label:
            c_color = 6  # 洋红
        else:
